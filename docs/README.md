...
| [0022](adr/0022-render-loop-hosting.md) | Window, event loop, and render-pass hosting — winit + wgpu + egui-wgpu + egui-winit, not eframe |
| [0023](adr/0023-radar-sampling-in-screen-space.md) | Radar sampling in screen space — full-screen triangle, per-pixel inverse map, `R8Uint` + LUT |
| [0024](adr/0024-gpu-adapter-selection.md) | GPU adapter selection — match the connected display, verify by bring-up, `RADAR_GPU` override; supersedes ADR-0022's `PowerPreference::LowPower` |
| [0025](adr/0025-bundled-overlay-geometry.md) | Bundled overlay geometry — build-time bake, no runtime parser (Q15); supersedes ADR-0006's parser clause |
| [0026](adr/0026-tile-http-boundary.md) | The tile HTTP boundary — one `http-ingest` engine, two policy crates (`s3-fetch`, `tile-fetch`); no redirects, ever (Q16); amends ADR-0014's scope boundaries. **Implementation deferred to post-v1.0 by ADR-0027**, except the `Bucket` enum |
| [0027](adr/0027-tile-image-decoding.md) | Tile image decoding — the tile subsystem is deferred to post-v1.0 and v1.0 ships a vector-only basemap; the codec answer (`png` + `jpeg-decoder`, dimension-gated, `catch_unwind`-contained) is recorded for when it returns (Q18, and Q5/Q7 with it) |
| [0028](adr/0028-city-labels.md) | City labels — Natural Earth 10m `populated_places` as a provisional, known-sparse source; a bundle label index + UTF-8 string table; a screen-space declutter pass; drawn by egui at `Order::Background`, which is compositing slot 9 (Q19); amends ADR-0025 §1–§4 and adds FR-MU-7 |
| [0029](adr/0029-primary-roads-simplification.md) | TIGER Primary Roads simplification — the layer measures 3,589,114 points, 8.0× the other three combined, and the 700 km filter keeps 100% of it; unsimplified it is 57.29 MB of GPU buffers against a 128 MB target. Douglas–Peucker at ε = 30 m, primary roads only — half a pixel at `view::MIN_M_PER_PX` and 8.3× finer than a 250 m gate, and deliberately *not* coupled to either constant (Q20); amends ADR-0025 §2 and §6 |
...
| [`stage-4-first-pixels.md`](plans/stage-4-first-pixels.md) | Implemented | Adds ADR-0022, ADR-0023, ADR-0024; the winit/wgpu/egui render loop draws the selected gridded product in azimuthal-equidistant projection with pan/zoom, keyboard product and sweep switching, a cursor readout, a legend, and a status bar. Its §16 Results is the authoritative account of what was measured — including the on-screen verification it could not complete. |
| [`stage-5-map-underlays.md`](plans/stage-5-map-underlays.md) | Implemented | Implements ADR-0025, ADR-0028, ADR-0029 (the baked overlay bundle, city labels, road simplification) and ADR-0026's `Bucket` enum: compositing layers 3–5, 8 and 9. No new open questions and no dependency delta. Its §18 Results is the authoritative account of what was measured. |
